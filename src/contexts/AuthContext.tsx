  ...
  const loginWithGoogle = async (preferredRole: UserRole = 'candidate') => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Popup blocked or failed, falling back to redirect flow...", popupError);
        ...
        try {
          ...
          console.error("Redirect flow failed too:", redirectError);
          throw new Error(
            "La connexion Google a échoué. Si vous utilisez un navigateur avec restrictions (comme Safari/iOS sans cookies tiers), veuillez autoriser les popups ou vous connecter par e-mail/mot de passe."
          );
        }
      }
    } catch (error) {
